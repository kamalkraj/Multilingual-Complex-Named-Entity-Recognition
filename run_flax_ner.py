...
def main():
    ...
    model = FlaxBertForTokenClassification.from_pretrained(
    ...
    def tokenize_and_align_labels(examples):
        ...
        labels = []
        tagged_positions = []
        pad = lambda x: x + [0] * (data_args.max_seq_length - len(x))
        for i, label in enumerate(examples[label_column_name]):
            ...
            label_ids.extend([-100] * (data_args.max_seq_length - len(label_ids)))
            labels.append(label_ids)
            tagged_positions_for_word = pad(tagged_positions_for_word)
            tagged_positions.append(tagged_positions_for_word)

        tokenized_inputs = tokenizer.pad(tokenized_inputs, padding="max_length", max_length=data_args.max_seq_length)
    ...
    def get_labels(y_pred, y_true):
        ...
        for pred, gold_label in zip(y_pred, y_true):
            predictions = []
            labels = []
            for pred_idx, gold_label_idx in zip(pred, gold_label):
                if gold_label_idx != -100:
                    predictions.append(label_list[pred_idx])
                    labels.append(label_list[gold_label_idx])
